...
room1.addDevice(new Lamp("Lamp1")); // success
room1.addDevice(new Lamp("Lamp2")); // success
sh.getRoomByName("living room"); // null
sh.addRoom(room1); // success
sh.addRoom(room1); // fail (same room)
sh.addRoom("qqq"); // fail (not a Room object)
sh.addRoom(room2); // success
sh.addRoom(room3); // success
sh.deleteRoom("Bathroom"); // confirmation required
sh.setNewRoomName(room2.getName(), "bedroom"); // success
console.log(sh.getInfo()); // just full info about home 
sh.getDeviceByName("Lamp1"); // success
sh.getDeviceByName("Lamp2"); // success
sh.setNewDeviceName("Lamp2", 12); // fail (incorrect name tyep)
sh.setNewDeviceName("Lamp2", "Lamp"); // success
sh.addNewDevice("living room", new TV("tv1")); // success
sh.addNewDevice("bedroom", new AirConditioner("AC1")); // success
sh.addNewDevice("bedroom", new AirConditioner("AC1")); // fail (same name)
var lamp = new Lamp("Lamp 2"); 
sh.addNewDevice("bedroom", new Lamp("Lamp 2")); // succeess

sh.deleteDevice("bedroom", lamp); // confirmation required

console.log(sh.getDevices()) // info about devices (names)
console.log(sh.getRooms()); // info about rooms (array of rooms
// all devices on|off
sh.turnOnAllDevices(); // success
console.log(sh.getInfo());
sh.turnOffAllDevices(); // success
console.log(sh.getInfo());

// air conditioner methods
var ac = sh.getDeviceByName("AC1"); // success
ac.turnOn("AC1"); // success
ac.setTemperature(24); // 24 degree
ac.timer(function () { ac.turnOff()}, 3000 ); // after 3 seconds ac will off, async
ac.setTemperature(38); // fail, incorrect temperature
ac.decreaseTemperatureBy1(); // 23
ac.increaseTemperatureBy1(); // 24

// tv methods
var tv1 = new TV("TV1");
sh.addNewDevice("bedroom", tv1); // success
var tv = sh.getDeviceByName("TV1"); // success
tv.timer(function () { tv.turnOff() }, 5000 ); // after 5 seconds tv will off, async
tv.setChannel(3); // success
tv.nextChannel(); // 4 channel
tv.previousChannel(); // 3 channel
tv.setVolume(9); // volume is 9
tv.decreaseVolumeBy1(); // volume is 8
tv.increaseVolumeBy1(); // volume is 9
tv.setRandomChannel(); // random channel

// lamp methods
var lamp = sh.getDeviceByName("Lamp1"); // success
lamp.setBrightness(3); // 3
lamp.timer(function () { lamp.turnOff()}, 7000 ); // after 7 seconds lamp will off, async
lamp.decreaseBrightnessBy1(); // 2
lamp.increaseBrightnessBy1(); // 3

// async functions will end soon (off ac, then tv, then lamp)